...
open class YPPhotoFiltersVC: UIViewController, IsMediaFilterVC, UIGestureRecognizerDelegate {
    ...
    fileprivate var filtersView = YPFiltersView()

    override open var prefersStatusBarHidden: Bool { return YPConfig.hidesStatusBar }
    override open func loadView() { view = filtersView }
    ...
    override open func viewDidLoad() {
        ...
        filtersView.imageView.image = inputPhoto.image
        ...
        DispatchQueue.global().async {
            ...
            DispatchQueue.main.async {
                self.filtersView.filterCollectionView.reloadData()
                self.filtersView.filterCollectionView.selectItem(at: IndexPath(row: 0, section: 0),
                                            animated: false,
                                            scrollPosition: UICollectionView.ScrollPosition.bottom)
                self.filtersView.filtersLoader.stopAnimating()
            }
        }
        
        // Setup of Collection View
        filtersView.filterCollectionView.register(YPFilterCollectionViewCell.self, forCellWithReuseIdentifier: "FilterCell")
        filtersView.filterCollectionView.dataSource = self
        filtersView.filterCollectionView.delegate = self
        ...
        filtersView.imageView.addGestureRecognizer(touchDownGR)
        filtersView.imageView.isUserInteractionEnabled = true
    }
    ...
    fileprivate func handleTouchDown(sender: UILongPressGestureRecognizer) {
        switch sender.state {
        case .began:
            filtersView.imageView.image = inputPhoto.originalImage
        case .ended:
            filtersView.imageView.image = currentlySelectedImageThumbnail ?? inputPhoto.originalImage
        default: ()
        }
    }
...
extension YPPhotoFiltersVC: UICollectionViewDelegate {
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        self.filtersView.imageView.image = currentlySelectedImageThumbnail
    }
}
