...
//                self.topBar.isUserInteractionEnabled = true
    ...
    func displayGridNextLevel() {
        let numberOfGrid = overlay.gridLinesCount
        if numberOfGrid == 0 {
            overlay.gridLinesCount = 2
        }
        else if numberOfGrid == 2 {
            overlay.gridLinesCount = 5
        }
        else if numberOfGrid == 5 {
            overlay.gridLinesCount = 8
        }
        else if numberOfGrid == 8 {
            overlay.gridLinesCount = 0
        }
    }
...
//        bottomView.size = CGSize(width: view.width, height: toolbar.height + angleRuler.height + margin)
        bottomView.size = CGSize(width: view.width, height: view.height - view.width)
...
//        let topHeight = topBar.isHidden ? view.safeAreaInsets.top : topBar.height
        ...
        defaultCropBoxCenter = CGPoint(x: view.width / 2.0, y: view.width / 2.0)
        defaultCropBoxSize = {
            var size: CGSize
...
//        scrollViewContainer.frame = CGRect(x: 0, y: topHeight, width: view.width, height: view.height - topHeight - bottomHeight)
        scrollViewContainer.frame = CGRect(x: 0, y: 0, width: view.width, height: view.width)
...
//        overlay.frame = backgroundView.bounds
        overlay.frame = scrollViewContainer.bounds
